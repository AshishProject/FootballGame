...
var updateButtonText = () =>{
    ...
    if(team1.runs.length == 6 && team2.runs.length == 6){
        ...
        result.textContent = team1.score === team2.score ? `Match is draw , Try again`: `${team1.score > team2.score? team1.name:team2.name} Wins`;
    }
    else{
        toss = toss === 1 ? 2 : 1;
    }
    ...
    button.textContent = `${toss%2 === 1 ? team1.name:team2.name} On Strike`;
};
...
var updateRuns = () =>{
    ...
    team1.runs.forEach((run,index)=>{
        run === 1 ? teamOneRunsElement[index].style.backgroundColor = "#00FF00" :teamOneRunsElement[index].style.backgroundColor = "#FF0000";
    });
    team2.runs.forEach((run,index)=>{
        run === 1 ? teamTwoRunsElement[index].style.backgroundColor = "#00FF00" : teamTwoRunsElement[index].style.backgroundColor = "#FF0000";
